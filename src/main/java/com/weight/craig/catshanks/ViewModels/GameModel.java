package com.weight.craig.catshanks.ViewModels;
...
import com.weight.craig.catshanks.BaseObjects.AbstractSprite;
...
public class GameModel {
    ...
    private void updateZoneBuffers(AbstractSprite sprite){
        if(sprite.getCollisionBox().isRect())
            updateZoneBuffers((int)sprite.getCollisionBox().getRect().left,
                    (int)sprite.getCollisionBox().getRect().top,
                    (int)sprite.getCollisionBox().getRect().right,
                    (int)sprite.getCollisionBox().getRect().bottom);
        else if(sprite.getCollisionBox().isCircle()){
            updateZoneBuffers((int)(sprite.getCollisionBox().getCircle().x() - sprite.getCollisionBox().getCircle().radius()),
                    (int)(sprite.getCollisionBox().getCircle().y() - sprite.getCollisionBox().getCircle().radius()),
                    (int)(sprite.getCollisionBox().getCircle().x() + sprite.getCollisionBox().getCircle().radius()),
                    (int)(sprite.getCollisionBox().getCircle().y() + sprite.getCollisionBox().getCircle().radius()));
        }
    }
            ...
            for(int i=0; i<PlayerZones.size();i++){
                ...
                for(int j=0; j < _PowerUps.get(i).size(); j++){
                    if(_PowerUps.get(i).get(j).isActive()){
                        if(collisionCheck(_PowerUps.get(i).get(j).getCollisionBox(),player.getCollisionBox())){
                ...
                if(!player.isInvincible()) {
                    ...
                    for (int j = 0; j < _Enemies.get(i).size(); j++) {
                        if(collisionCheck(_Enemies.get(i).get(j).getCollisionBox(),player.getCollisionBox())){
                    ...
                    for (int j = 0; j < _EnemyProjectiles.get(i).size(); j++) {
                        if (_EnemyProjectiles.get(i).get(j).getActive()) {
                            if(collisionCheck(player.getCollisionBox(),_EnemyProjectiles.get(i).get(j).getCollisionBox())){
            ...
            for(int j=0; j < _Enemies.get(i).size();j++){
                for(int k=0; k < _FriendlyProjectiles.get(i).size(); k++){
                    if(_FriendlyProjectiles.get(i).get(k).getActive()){
                        ...
                        if(collisionCheck(_Enemies.get(i).get(j).getCollisionBox(), _FriendlyProjectiles.get(i).get(k).getCollisionBox())){
    ...
    private void handleCollisions(){
        ...
        if(!playerdead){
            for(int i=0; i<PowerUps.size();i++){
                if(PowerUps.get(i).isActive()){
                    if(collisionCheck(PowerUps.get(i).getCollisionBox(),player.getCollisionBox())){
                        Audio_Manager.getInstance().playSound(pUpSound);
                        PowerUps.get(i).AI(0);
                    }
                }
            ...
            if(enemy.isActive()){
                ...
                for(Projectile projectile: FriendlyProjectiles){
                    if(projectile.getActive() & projectile.isCollidable()){
                        //Check for collisions between enemies and projectiles
                        if(collisionCheck(enemy.getCollisionBox(),projectile.getCollisionBox())){
        ...
        if(!player.isInvincible() && !playerdead){
            for(Projectile projectile: EnemyProjectiles){
                if(projectile.getActive()){
                    ...
                    if(collisionCheck(player.getCollisionBox(),projectile.getCollisionBox())){
                        Audio_Manager.getInstance().playSound(hit3);
    ...
    private void CollisionEnemyandPlayer(Enemy enemy){
        ...
        if(!player.isInvincible() && !playerdead){
            //Check for collisions between player and enemies.
            if(collisionCheck(enemy.getCollisionBox(),player.getCollisionBox())){
                //If an enemy should die on collision
